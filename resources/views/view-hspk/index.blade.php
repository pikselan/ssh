...
function format ( d ) {
      ...
      d.sub_turunan.forEach((item, index) => {
            listSub += '<tr><td></td><td></td><td>' + nullToString(item.col_b) + '</td><td>' + nullToString(item.col_c) + '</td><td>' + nullToString(item.col_d) + '</td><td>' + nullToString(item.col_e) + '</td><td>' + numberWithCommas(nullToString(item.col_f)) + '</td><td>' + numberWithCommas(nullToString(item.col_g)) + '</td><td>' + numberWithCommas(nullToString(item.col_h)) + '</td></tr>';
      });

      return '<table class="table" style="width:auto;border: 2px solid #dfdfdf;">'+
            '<tbody>'
            +listSub+
...
$(document).ready(function(){
      var table = $('#empTable').DataTable({
            ...
            serverSide: true,
            searchDelay: 2000,
            ajax: "{{route('getDataHspk')}}?select_periode={{$select_periode}}",
            columns: [
                  {
            ...
            buttons: [
                  ...
                  $.extend( true, {}, {
                        ...
                        title: 'Data HSPK Kabupaten Pesawaran',
                              ...
                              // Remove detail-control column
                                    ...
                                    if (line[0].style !== 'tableHeader' && line[0].style !== 'tableFooter') {
                                          ...
                                          data.sub_turunan.forEach(function(sub, i){
                                                if(letterCounter(nullToString(sub.col_b)) > 4) {
                                                      newBody.push([
                                                            {text: '', style:''},
                                                            {text: nullToString(sub.col_b), style:'', colSpan: 3},
                                                            {text: '', style:''},
                                                            {text: '', style:''},
                                                            {text: '', style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_f)), style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_g)), style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_h)), style:''},
                                                      ]);
                                                } else {
                                                      newBody.push([
                                                            {text: '', style:''},
                                                            {text: nullToString(sub.col_b), style:''},
                                                            {text: nullToString(sub.col_c), style:''},
                                                            {text: nullToString(sub.col_d), style:''},
                                                            {text: nullToString(sub.col_e), style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_f)), style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_g)), style:''},
                                                            {text: numberWithCommas(nullToString(sub.col_h)), style:''},
                                                      ]);
